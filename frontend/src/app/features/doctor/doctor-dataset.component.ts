...
export class DoctorDatasetComponent implements OnInit, OnDestroy {
  searchTerm = '';
  records: DoctorDatasetRecord[] = [];
  isLoading = false;
  ...
  loadDataset(): void {
    this.isLoading = true;
    this.apiService.get<{ records: DoctorDatasetRecord[]; total: number }>('/dataset/records')
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (response: { records: DoctorDatasetRecord[]; total: number }) => {
